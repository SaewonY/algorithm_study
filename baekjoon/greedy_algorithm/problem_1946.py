...
def solution(test_num):
    ...
    for i in range(test_num):
        ...
        score_list.append([document_score, interview_score])

    sorted_score = sorted(score_list, key=lambda item: item[0])

    count = 0
    min_score = 12353125
    for i, case in enumerate(sorted_score):
        
        if case[1] < min_score:
            min_score = case[1]
        if case[0] == 1 or case[1] == 1:
            count += 1
            continue
        if case[1] > min_score:
            continue
